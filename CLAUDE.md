...
Решения по ходу — `docs/adr/`. Перед работой прочитай README.md и оба ADR.

## Статус (на 2026-09-25)

- Этап 0 — `app/public/probe.html` готова; **ждём результатов проверки с телефона** (мобильная сеть РФ) →
  заполнить `docs/adr/0001-feasibility.md`. От них зависит этап 4.
...
## Разовая настройка, если ещё не сделана

1. Папка `github-workflows/` → перенести файлы в `.github/workflows/`, пустую папку удалить.
2. В корне: `git init`, первый коммит, репозиторий `music-catalog` на GitHub, push в `main`.
